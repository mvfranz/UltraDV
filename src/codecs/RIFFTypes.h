...
{ 
	...
	uint32 Width;                 // Width of video frame in pixels
	uint32 Height;                // Height of video frame in pixels
...
typedef struct
{
	...
	uint32 Rate;           			// L rate
	uint32 AverageDataRate;         // L average bytes/sec
	uint16 BlockAlign;	  			// S block size of data
	uint32 Size;           			// S size
	uint32 ExtensionSize;       	// S size of following extenions - if any
	uint32 SamplesPerBlock;	  		// S Used by MSADPCM and Intel DVI
	uint32 NumCoefficients;	  		// S Used by MSADPCM  num of follow sets
	WaveCoefficient *Coefficients;	// Used by MSADPCM  coef1, coef2
